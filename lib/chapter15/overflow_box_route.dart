...
/**
 * OverflowBox是Flutter中的一个布局小部件（Widget），可允许其子小部件"溢出"父级小部件的约束。
 *  在标准的Flutter布局规则中，小部件的大小通常被其父小部件约束，但是OverflowBox提供了一种方式，
 *  让子小部件可以在某个方向（水平或垂直）上绘制超出父小部件所提供的空间限制。
    ...
    OverflowBox并不改变其子小部件的实际大小，只是改变了子小部件可以绘制的空间。这意味着OverflowBox内的小部件可以绘制到父小部件以外，
    但它仍然按照原始大小进行布局计算。当使用OverflowBox时，要考虑小部件溢出的部分是否会被其他UI元素覆盖，或是否会使用户界面看起来杂乱无章。

    Optimize: 仅仅是视觉效果，OverflowBox来承载上层父容器的约束
 */
